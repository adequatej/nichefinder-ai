...
This environment did not have a live Postgres available while this
package was built, so the table below is **not** even from
`app/ingest/sample_data.py`'s fixture — it's from a small synthetic
harness built by hand (6 channels, ~54 candidate videos, hand-planted
view-count outliers so at least a few labels come out positive) used
only to exercise every code path — export → labels → features →
folds → both baselines → the PyTorch model — end to end without a
crash, and to check that two seeded runs reproduce.

| model | folds | PR-AUC | ROC-AUC | precision@k (k≈11) |
|---|---|---|---|---|
| logistic_regression | 5 | 0.230 ± 0.068 | 0.519 ± 0.213 | 0.111 |
| hist_gradient_boosting | 5 | 0.167 ± 0.084 | 0.350 ± 0.242 | 0.111 |
| pytorch_mlp | 5 | 0.421 ± 0.313 | 0.688 ± 0.230 | 0.111 |

**This table means nothing predictively** — the input data behind it
is hand-fabricated for code coverage, not a real corpus, and n is tiny
(≈11 candidates per fold). It is proof the pipeline runs, is
reproducible (`train_torch.py`'s `set_seed` also pins
`torch.set_num_threads(1)`; without that, CPU matmul's
thread-scheduling-dependent reduction order made even seeded reruns
differ — two seeded runs of the harness above now produce bit-identical
metrics), and produces a well-formed comparison table — not a claim
about which model is better.

**The real comparison table** gets generated by running
`python -m src.export_dataset` against a Postgres loaded with either
`make bootstrap-sample` (the deterministic keyless fixture — still
label-wise meaningless per the note above, but a true test of the full
pipeline against the actual sample corpus) or, eventually, real
bootstrap data, followed by `python -m src.train_baselines` and
`python -m src.train_torch`. Neither has been run in this environment.
